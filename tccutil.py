...
def display_help(error_code=None):
    ...
    print ("  sudo %s [--insert | --remove | --enable | --disable] <bundle_id | cli_path> [--user <user>] [--verbose]" % (
    util_name))
    print ("")
    print ("Pass through reset command to built-in OS X utility:")
    print ("  %s reset <Accessibility | AddressBook | Calendar | CoreLocationAgent | Facebook | Reminders | Twitter>" % (
    util_name))
    print ("")
    print ("Options:")
    ...
    if error_code is not None: sys.exit(error_code)
...
def insert_client(client):
    ...
    if osx_version >= version('10.11'):  # El Capitan or higher.
        c.execute("INSERT or REPLACE INTO access VALUES('kTCCServiceAccessibility','%s',%s,1,1,NULL,NULL)" % (
        client, client_type))
    else:  # Yosemite or lower.
        c.execute(
            "INSERT or REPLACE INTO access VALUES('kTCCServiceAccessibility','%s',%s,1,1,NULL)" % (client, client_type))
    commit_changes()
...
def main():
    ...
    try:
        ...
        opts, args = getopt.getopt(sys.argv[1:], "hlvui:r:e:d:u",
                                   ['help', 'version', 'list', 'verbose', 'insert=', 'remove=', 'enable=', 'disable=',
                                    'user'])
